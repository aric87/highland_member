...
var fileFolderPath = '../../views/tunes/';
...
module.exports = function(app,multipartyMiddleware,fs) {
    ...
    app.get('/tunes/:name', isLoggedIn, function(req, res) {
        Song.find({name:req.params.name},function(err, tune) {
            if (err) {
                console.log(err);
                return;
            }
        ...
        fs.readFile(req.files.file.path, function (err, data) {
            ...
            fs.writeFile(createDir, data, function (err) {
                if (err) {
                    ...
                    Document.findOne({name: req.body.name}, function (err, tune) {
                        if (err) {
                            console.log(err);
                            return;
                        }
        ...
        fs.readFile(req.files.file.path, function (err, data) {
            ...
            fs.writeFile(createDir, data, function (err) {
                if (err) {
                    ...
                    Document.create({name: req.body.name,file:'/tunes/'+req.files.file.name}, function (err, document) {
                        if (err) {
                            console.log(err);
                            return;
                        }
